...
static NativeValueDecl v5 = {.name="__function_type", .value = NUMBER_VAL(OBJ_FUNCTION + VAL_OBJ)};
static NativeValueDecl v6 = {.name="__closure_type", .value = NUMBER_VAL(OBJ_CLOSURE + VAL_OBJ)};
static NativeValueDecl v7 = {.name="__up_value_type", .value = NUMBER_VAL(OBJ_UPVALUE + VAL_OBJ)};
static NativeValueDecl v8 = {.name="__class_type", .value = NUMBER_VAL(OBJ_CLASS + VAL_OBJ)};
static NativeValueDecl v9 = {.name="__instance_type", .value = NUMBER_VAL(OBJ_INSTANCE + VAL_OBJ)};
static NativeValueDecl v10 = {.name="__method_type", .value = NUMBER_VAL(OBJ_INSTANCE_METHOD + VAL_OBJ)};
static NativeValueDecl v11 = {.name="__module_type", .value = NUMBER_VAL(OBJ_MODULE + VAL_OBJ)};
  ...
  switch (type) {
    ...
    case VAR_DECL_STMT: {
      ...
      if (local_index != -1) {
        ...
        // global
        int var_index = global_variable_decl(compiler, statement->var_decl.name, statement->var_decl.initializer, true);
    ...
    case FUNC_STMT: {
      ...
      if (local_index != -1) {
        ...
        // global
        int var_index =
            global_variable_decl(compiler, statement->function_stmt.name, statement->function_stmt.function, true);
        VARIABLE_OPERATION(compiler, var_index, statement->line, OP_DEFINE_GLOBAL);
      }
      return;
    }
