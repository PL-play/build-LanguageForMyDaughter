    ...
    switch (OBJ_TYPE(callee)) {
      ...
      case OBJ_NATIVE: {
        if (arg_count != ((ObjNative *) AS_OBJ(callee))->arity) {
          runtime_error(vm, "Expect %d arguments but got %d", ((ObjNative *) AS_OBJ(callee))->arity, arg_count);
          return false;
        }
      ...
      case OBJ_CLASS: {
        ...
        // find init method
        ObjString
            obj_string = {.obj=(Obj) {.type=OBJ_STRING}, .length=init_len, .string = INIT_METHOD_NAME, .hash=init_hash};
        Value init =
        ...
        if (init.type == VAL_UNDEFINED) {
          ...
          // call init method
          return is_func ? call_function(vm, (ObjFunction *) init.obj, arg_count)
      ...
      case OBJ_INSTANCE_METHOD: {
        ...
        return method->method->type == OBJ_FUNCTION ? call_function(vm, (ObjFunction *) method->method, arg_count)
